...
-type valid_message(VSB) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, VSB}.

-type websocket_message(VSC) :: {valid, valid_message(VSC)} | invalid.
...
-type handler_message(VSD) :: {internal, gramps@websocket:frame()} | {user, VSD}.

-type websocket_state(VSE) :: {websocket_state,
        bitstring(),
        VSE,
        gleam@option:option(gramps@websocket@compression:compression())}.

-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 56).
...
-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 264).
-spec get_messages(
    bitstring(),
...
-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 354).
...
-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 361).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(VTM))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(VTM))).
...
-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 277).
-spec apply_frames(
    list(gramps@websocket:frame()),
    fun((VTC, websocket_connection(), handler_message(VTD)) -> gleam@otp@actor:next(VTD, VTC)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(VTD), VTC),
    fun((VTC) -> nil)
) -> gleam@otp@actor:next(websocket_message(VTD), VTC).
...
-file("/var/www/build/packages/mist/src/mist/internal/websocket.gleam", 82).
-spec initialize_connection(
    fun((websocket_connection()) -> {VSN,
        gleam@option:option(gleam@erlang@process:selector(VSO))}),
    fun((VSN) -> nil),
    fun((VSN, websocket_connection(), handler_message(VSO)) -> gleam@otp@actor:next(VSO, VSN)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(VSO))} | {error, nil}.
