...
-file("/var/www/build/packages/mist/src/mist/internal/handler.gleam", 29).
-spec new_state(
    gleam@erlang@process:subject(mist@internal@http2@handler:message())
) -> state().
new_state(Subj) ->
...
-file("/var/www/build/packages/mist/src/mist/internal/handler.gleam", 33).
-spec init(any()) -> {state(),
...
-file("/var/www/build/packages/mist/src/mist/internal/handler.gleam", 42).
-spec with_func(
