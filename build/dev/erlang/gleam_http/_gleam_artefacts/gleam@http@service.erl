...
-file("/var/www/build/packages/gleam_http/src/gleam/http/service.gleam", 17).
-spec map_response_body(
    fun((JUB) -> gleam@http@response:response(JTN)),
    fun((JTN) -> JTO)
) -> fun((JUB) -> gleam@http@response:response(JTO)).
map_response_body(Service, Mapper) ->
    fun(Req) -> _pipe = Req,
        _pipe@1 = Service(_pipe),
        gleam@http@response:map(_pipe@1, Mapper) end.

-file("/var/www/build/packages/gleam_http/src/gleam/http/service.gleam", 26).
-spec prepend_response_header(
    fun((JUF) -> gleam@http@response:response(JUH)),
    binary(),
    binary()
) -> fun((JUF) -> gleam@http@response:response(JUH)).
...
-file("/var/www/build/packages/gleam_http/src/gleam/http/service.gleam", 34).
-spec ensure_post(gleam@http@request:request(JTS)) -> {ok,
        gleam@http@request:request(JTS)} |
    {error, nil}.
ensure_post(Req) ->
...
-file("/var/www/build/packages/gleam_http/src/gleam/http/service.gleam", 41).
...
-file("/var/www/build/packages/gleam_http/src/gleam/http/service.gleam", 52).
-spec method_override(fun((gleam@http@request:request(JVO)) -> JVW)) -> fun((gleam@http@request:request(JVO)) -> JVW).
