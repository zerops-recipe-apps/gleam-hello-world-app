...
-file("/var/www/build/packages/glenvy/src/glenvy/internal/lexer.gleam", 22).
...
-file("/var/www/build/packages/glenvy/src/glenvy/internal/lexer.gleam", 53).
-spec tokenize(binary()) -> {ok, list(nibble@lexer:token(token_kind()))} |
    {error, nibble@lexer:error()}.
tokenize(Input) ->
    nibble@lexer:run(Input, lexer()).

-file("/var/www/build/packages/glenvy/src/glenvy/internal/lexer.gleam", 58).
