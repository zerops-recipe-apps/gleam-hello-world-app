...
-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 51).
...
-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 62).
...
-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 24).
-spec new() -> string_builder().
new() ->
    gleam_stdlib:identity([]).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 77).
-spec from_strings(list(binary())) -> string_builder().
from_strings(Strings) ->
    gleam_stdlib:identity(Strings).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 89).
-spec concat(list(string_builder())) -> string_builder().
concat(Builders) ->
    gleam_stdlib:identity(Builders).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 101).
-spec from_string(binary()) -> string_builder().
from_string(String) ->
    gleam_stdlib:identity(String).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 32).
-spec prepend(string_builder(), binary()) -> string_builder().
prepend(Builder, Prefix) ->
    append_builder(from_string(Prefix), Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 43).
-spec append(string_builder(), binary()) -> string_builder().
append(Builder, Second) ->
    append_builder(Builder, from_string(Second)).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 114).
-spec to_string(string_builder()) -> binary().
to_string(Builder) ->
    unicode:characters_to_binary(Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 124).
-spec byte_size(string_builder()) -> integer().
byte_size(Builder) ->
    erlang:iolist_size(Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 134).
...
-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 143).
-spec lowercase(string_builder()) -> string_builder().
lowercase(Builder) ->
    string:lowercase(Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 154).
-spec uppercase(string_builder()) -> string_builder().
uppercase(Builder) ->
    string:uppercase(Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 164).
-spec reverse(string_builder()) -> string_builder().
reverse(Builder) ->
    string:reverse(Builder).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 191).
-spec do_split(string_builder(), binary()) -> list(string_builder()).
do_split(Iodata, Pattern) ->
    string:split(Iodata, Pattern, all).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 182).
-spec split(string_builder(), binary()) -> list(string_builder()).
split(Iodata, Pattern) ->
    do_split(Iodata, Pattern).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 202).
...
-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 227).
-spec is_equal(string_builder(), string_builder()) -> boolean().
is_equal(A, B) ->
    string:equal(A, B).

-file("/var/www/build/packages/gleam_stdlib/src/gleam/string_builder.gleam", 251).
