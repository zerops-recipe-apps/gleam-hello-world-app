...
-type pool(REF, REG) :: {pool,
        fun((glisten@internal@handler:loop_message(REF), REG, glisten@internal@handler:connection(REF)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(REF), REG)),
        integer(),
        fun((glisten@internal@handler:connection(REF)) -> {REG,
            gleam@option:option(gleam@erlang@process:selector(REF))}),
        gleam@option:option(fun((REG) -> nil)),
        glisten@transport:transport()}.

-file("/var/www/build/packages/glisten/src/glisten/internal/acceptor.gleam", 38).
-spec start(
    pool(any(), any()),
...
-file("/var/www/build/packages/glisten/src/glisten/internal/acceptor.gleam", 123).
-spec start_pool(
    pool(any(), any()),
    glisten@transport:transport(),
    integer(),
