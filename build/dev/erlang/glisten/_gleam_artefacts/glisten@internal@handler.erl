...
-type message(QSI) :: {internal, internal_message()} | {user, QSI}.

-type loop_message(QSJ) :: {packet, bitstring()} | {custom, QSJ}.

-type loop_state(QSK, QSL) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(QSK)),
        glisten@transport:transport(),
        QSL}.

-type connection(QSM) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(QSM))}.

-type handler(QSN, QSO) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(QSN), QSO, connection(QSN)) -> gleam@otp@actor:next(loop_message(QSN), QSO)),
        fun((connection(QSN)) -> {QSO,
            gleam@option:option(gleam@erlang@process:selector(QSN))}),
        gleam@option:option(fun((QSO) -> nil)),
        glisten@transport:transport()}.

-file("/var/www/build/packages/glisten/src/glisten/internal/handler.gleam", 73).
-spec start(handler(QSY, any())) -> {ok,
        gleam@erlang@process:subject(message(QSY))} |
