...
-file("/var/www/build/packages/gleam_erlang/src/gleam/erlang/node.gleam", 10).
-spec self() -> node_().
self() ->
    erlang:node().

-file("/var/www/build/packages/gleam_erlang/src/gleam/erlang/node.gleam", 23).
-spec visible() -> list(node_()).
visible() ->
    erlang:nodes().

-file("/var/www/build/packages/gleam_erlang/src/gleam/erlang/node.gleam", 44).
-spec connect(gleam@erlang@atom:atom_()) -> {ok, node_()} |
    {error, connect_error()}.
connect(Node) ->
    gleam_erlang_ffi:connect_node(Node).

-file("/var/www/build/packages/gleam_erlang/src/gleam/erlang/node.gleam", 51).
-spec send(node_(), gleam@erlang@atom:atom_(), any()) -> nil.
send(Node, Name, Message) ->
    erlang:send({Name, Node}, Message),
    nil.

-file("/var/www/build/packages/gleam_erlang/src/gleam/erlang/node.gleam", 62).
