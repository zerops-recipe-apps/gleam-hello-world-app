...
-type message(JYP) :: {message, JYP} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(JYQ, JYR) :: {continue,
        JYR,
        gleam@option:option(gleam@erlang@process:selector(JYQ))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(JYS, JYT) :: {ready, JYS, gleam@erlang@process:selector(JYT)} |
    {failed, binary()}.

-type self(JYU, JYV) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        JYU,
        gleam@erlang@process:subject(JYV),
        gleam@erlang@process:selector(message(JYV)),
        gleam@otp@system:debug_state(),
        fun((JYV, JYU) -> next(JYV, JYU))}.

-type spec(JYW, JYX) :: {spec,
        fun(() -> init_result(JYW, JYX)),
        integer(),
        fun((JYX, JYW) -> next(JYX, JYW))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(JYY) :: {ack,
        {ok, gleam@erlang@process:subject(JYY)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 178).
-spec continue(JZF) -> next(any(), JZF).
continue(State) ->
    {continue, State, none}.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 186).
-spec with_selector(next(JZJ, JZK), gleam@erlang@process:selector(JZJ)) -> next(JZJ, JZK).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 259).
-spec exit_process(gleam@erlang@process:exit_reason()) -> gleam@erlang@process:exit_reason().
exit_process(Reason) ->
    Reason.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 295).
-spec selecting_system_messages(gleam@erlang@process:selector(message(JZV))) -> gleam@erlang@process:selector(message(JZV)).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 264).
-spec receive_message(self(any(), JZR)) -> message(JZR).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 308).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 415).
-spec init_selector(
    gleam@erlang@process:subject(KEJ),
    gleam@erlang@process:selector(KEJ)
) -> gleam@erlang@process:selector(message(KEJ)).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 318).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 374).
-spec initialise_actor(
    spec(any(), KAM),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(KAM)} |
        {error, gleam@erlang@process:exit_reason()})
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 444).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 466).
-spec start_spec(spec(any(), KAZ)) -> {ok, gleam@erlang@process:subject(KAZ)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 514).
-spec start(KBF, fun((KBG, KBF) -> next(KBG, KBF))) -> {ok,
        gleam@erlang@process:subject(KBG)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 529).
-spec send(gleam@erlang@process:subject(KBM), KBM) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 542).
-spec call(
    gleam@erlang@process:subject(KBO),
    fun((gleam@erlang@process:subject(KBQ)) -> KBO),
    integer()
) -> KBQ.
