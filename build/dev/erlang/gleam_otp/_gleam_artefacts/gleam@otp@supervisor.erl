...
-type spec(KNF, KNG) :: {spec,
        KNF,
        integer(),
        integer(),
        fun((children(KNF)) -> children(KNG))}.

-opaque children(KNH) :: {ready, starter(KNH)} | {failed, child_start_error()}.

-opaque child_spec(KNI, KNJ, KNK) :: {child_spec,
        fun((KNJ) -> {ok, gleam@erlang@process:subject(KNI)} |
            {error, gleam@otp@actor:start_error()}),
        fun((KNJ, gleam@erlang@process:subject(KNI)) -> KNK)}.
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 87).
-spec start_child(child_spec(any(), KNR, KNS), KNR) -> {ok, child(KNS)} |
    {error, child_start_error()}.
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 106).
-spec shutdown_child(
    gleam@erlang@process:pid_(),
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 110).
-spec perform_instruction_for_child(
    KOF,
    instruction(),
    child_spec(any(), KOF, KOH),
    child(KOH)
) -> {ok, {child(KOH), instruction()}} | {error, child_start_error()}.
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 132).
-spec add_child_to_starter(
    starter(KOP),
    child_spec(any(), KOP, KOS),
    child(KOS)
) -> starter(KOS).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 164).
-spec start_and_add_child(starter(KOY), child_spec(any(), KOY, KPB)) -> children(KPB).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 178).
-spec add(children(KPG), child_spec(any(), KPG, KPJ)) -> children(KPJ).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 206).
-spec supervisor(
    fun((KPO) -> {ok, gleam@erlang@process:subject(KPP)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KPP, KPO, KPO).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 228).
-spec worker(
    fun((KPW) -> {ok, gleam@erlang@process:subject(KPX)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KPX, KPW, KPW).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 241).
-spec returning(
    child_spec(KQE, KQF, any()),
    fun((KQF, gleam@erlang@process:subject(KQE)) -> KQL)
) -> child_spec(KQE, KQF, KQL).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), KQQ)) -> gleam@otp@actor:init_result(state(KQQ), message()).
init(Spec) ->
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 305).
-spec handle_exit(gleam@erlang@process:pid_(), state(KQW)) -> gleam@otp@actor:next(message(), state(KQW)).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 345).
-spec loop(message(), state(KRB)) -> gleam@otp@actor:next(message(), state(KRB)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 377).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 408).
...
-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 438).
-spec application_stopped() -> application_stop().
application_stopped() ->
    gleam_otp_external:application_stopped().

-file("/var/www/build/packages/gleam_otp/src/gleam/otp/supervisor.gleam", 457).
