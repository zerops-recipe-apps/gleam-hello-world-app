...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 20).
-spec strong_random_bytes(integer()) -> bitstring().
strong_random_bytes(A) ->
    crypto:strong_rand_bytes(A).

-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 73).
-spec new_hasher(hash_algorithm()) -> hasher().
new_hasher(Algorithm) ->
    gleam_crypto_ffi:hash_init(Algorithm).

-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 81).
-spec hash_chunk(hasher(), bitstring()) -> hasher().
hash_chunk(Hasher, Chunk) ->
    crypto:hash_update(Hasher, Chunk).

-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 89).
-spec digest(hasher()) -> bitstring().
digest(Hasher) ->
    crypto:hash_final(Hasher).

-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 49).
...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 100).
-spec hmac(bitstring(), hash_algorithm(), bitstring()) -> bitstring().
hmac(Data, Algorithm, Key) ->
    gleam_crypto_ffi:hmac(Data, Algorithm, Key).

-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 113).
...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 106).
...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 143).
...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 132).
...
-file("/var/www/build/packages/gleam_crypto/src/gleam/crypto.gleam", 163).
